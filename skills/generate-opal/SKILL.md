...
Both functions work directly in `pick_col` and bind the output's valid-from / valid-to columns, so you never need to reference a temporal column by its schema name — even there. They also work directly in `sort` direction functions — `sort desc(row_start_time())` and `sort asc(row_end_time())` are valid.
